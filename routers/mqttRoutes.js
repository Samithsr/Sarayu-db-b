...
const AllTopicsModel = require("../src/models/all-mqtt-messages");
const TopicsModel = require("../src/models/topics-model");
...
router.post("/subscribe-to-all", async (req, res) => {
  try {
    // Get all available topics from TopicsModel
    ...
    // Save each topic to SubscribedTopic database
    for (const topicData of topics) {
      const existingSubscription = await SubscribedTopic.findOne({ topic: topicData.topic });
      if (!existingSubscription) {
        await SubscribedTopic.create({ topic: topicData.topic });
      }
    }

    // Subscribe to each topic via MQTT
...
router.post("/unsubscribe-from-all", async (req, res) => {
  try {
    ...
    // Remove unsubscribed topics from SubscribedTopic database
    for (const topicName of unsubscribedTopics) {
      await SubscribedTopic.deleteOne({ topic: topicName });
    }
